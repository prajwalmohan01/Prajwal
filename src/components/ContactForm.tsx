...
import { Send, Mail, Phone, MapPin, Linkedin, Instagram, Globe, CheckCircle2 } from 'lucide-react';
...
export const ContactForm: React.FC<ContactFormProps> = ({ onToast }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulate EmailJS transmission delay
    setTimeout(() => {
      setIsSubmitting(false);
      onToast('Message Sent Successfully!', `Thank you ${formData.name}. Prajwal will reply to ${formData.email} shortly.`);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-5 flex flex-col justify-between px-5 py-6 sm:px-6 sm:py-6 rounded-2xl bg-slate-900 border border-slate-800 shadow-md space-y-6">
        <div>
          <span className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">
            Get In Touch
          </span>
          <h3 className="text-2xl font-bold text-white mt-1">Let's Connect & Build Together</h3>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-slate-800/60 border border-slate-700/60">
            ...
            <div>
              ...
              <a href={`mailto:${PERSONAL_INFO.email}`} className="text-xs font-semibold text-white hover:text-blue-400 transition-colors">
                {PERSONAL_INFO.email}
              </a>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-slate-800/60 border border-slate-700/60">
            ...
            <div>
              ...
              <a href={`tel:${PERSONAL_INFO.phone}`} className="text-xs font-semibold text-white hover:text-blue-400 transition-colors">
                {PERSONAL_INFO.phone}
              </a>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-slate-800/60 border border-slate-700/60">
            <div className="p-2.5 rounded-xl bg-slate-800 border border-slate-700 text-purple-400">
              <MapPin className="w-5 h-5" />
            </div>
            <div>
              <p className="text-[10px] font-mono uppercase text-slate-500">Location</p>
              <p className="text-xs font-semibold text-white">{PERSONAL_INFO.location}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-800/60 border border-slate-700/60 space-y-2">
          <div className="flex items-center justify-between text-xs text-slate-400">
            <span className="flex items-center gap-1 font-mono"><Globe className="w-3.5 h-3.5 text-blue-400" /> Bengaluru, KA</span>
            <span className="text-[10px] text-emerald-400 font-semibold uppercase">Open to relocation</span>
          </div>

          <div className="w-full h-24 rounded-xl bg-black/60 overflow-hidden relative border border-slate-700 flex items-center justify-center">
      ...
      {/* Form Bento Card */}
      <div className="lg:col-span-7 px-5 py-6 sm:px-6 sm:py-6 rounded-2xl bg-slate-900 border border-slate-800 shadow-md">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-xl bg-slate-800/80 border text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all ${
